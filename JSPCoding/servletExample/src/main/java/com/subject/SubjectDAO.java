...
public class SubjectDAO {
    ...
    public boolean subjectUpdate(SubjectVO svo) {
        ...
        try ( Connection conn = getConnection();
            ...
            pstmt.setString(1, svo.getSubjectName()); // 첫번째 ?(바인딩변수) 설정값 - 학과번호
            pstmt.setString(2, svo.getSubjectNumber());   // 두번째 ?(바인딩변수) 설정값 - 학과명
    ...
    public ArrayList<SubjectVO> subjectSearch(SubjectVO svo) {
	    ...
	    try(Connection conn = getConnection();
	        ...
	        while(rs.next()) {
	            //한 행의 학과 정보를 저장할 VO 객체 생성
	        	SubjectVO svo1 = new SubjectVO();
	            //한 행의 학과 정보를 VO 객체에  저장
	            svo1.setNo(rs.getInt("no"));
	            svo1.setSubjectNumber(rs.getString("s_num"));  
	            svo1.setSubjectName(rs.getString("s_name"));
    ...
    public boolean subjectDelete(SubjectVO svo) {
        StringBuffer sql = new StringBuffer();
        sql.append("delete from subject where no = ?"); //s_num을 기준으로 삭제가능. where s_num = ?
